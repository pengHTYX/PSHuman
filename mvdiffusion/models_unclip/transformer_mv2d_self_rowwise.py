...
import pdb
from .attn_processors import IPCDAttention, RowwiseMVAttention, IPCrossAttn
...
class BasicMVTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        # add cross domain attn to self attn
            self.attn1 = IPCDAttention(
                ...
                upcast_attention=upcast_attention,
                # processor=JointCDAttnProcessor()
            )
        else:
        ...
        if self.rowwise_attention:
            ...
            self.attn_mv = RowwiseMVAttention(
                    query_dim=dim,
                    heads=num_attention_heads,
                    dim_head=attention_head_dim,
                    dropout=dropout,
        ...
        if cross_attention_dim is not None or double_self_attention:
            ...
            # nn.init.zeros_(self.attn2.to_out_ip[0].weight.data)
        else:
            self.norm2 = None
            self.attn2 = None
    ...
    def forward(
        ...
        hidden_states = rearrange(hidden_states, '(b v) n c -> b v n c', v=self.num_views)
        body_hidden_states, face_hidden_states = \
            rearrange(hidden_states[:, :-1, :, :], 'b v n c -> (b v) n c'), hidden_states[:, -1:, :, :]
        if self.rowwise_attention:
            norm_hidden_states = (
                self.norm_mv(body_hidden_states, timestep) if self.use_ada_layer_norm else self.norm_mv(body_hidden_states)
            )
            attn_output = self.attn_mv(
                norm_hidden_states,
                encoder_hidden_states=encoder_hidden_states if self.only_cross_attention else None,
                attention_mask=attention_mask,
                num_views=self.num_views-1,
                multiview_attention=self.multiview_attention,
                cd_attention_mid=self.cd_attention_mid,
                **cross_attention_kwargs,
                )
            body_hidden_states = attn_output + body_hidden_states 
        hidden_states = rearrange(torch.cat([
            rearrange(body_hidden_states, '(b v) n c -> b v n c', v=self.num_views-1), face_hidden_states], dim=1), 'b v n c -> (b v) n c')
        ...
        if self.attn2 is not None:
            ...
            encoder_hidden_states = torch.cat([encoder_hidden_states, dino_feature], dim=1) if dino_feature is not None else encoder_hidden_states  
